...
impl<T: 'static> EventLoop<T> {
  ...
  fn new_gtk() -> Result<EventLoop<T>, Box<dyn Error>> {
    let context = MainContext::default();
    let application_id = env::var("TAO_UNIX_APPLICATION_ID")
      .ok()
      .map(|value| value.trim().to_string())
      .filter(|value| !value.is_empty());
    glib::set_prgname(application_id.as_deref());
    let app = gtk::Application::new(application_id.as_deref(), gio::ApplicationFlags::empty());
    let app_ = app.clone();
